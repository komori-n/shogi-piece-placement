...
namespace {
  ...
  int count_pawn_like(const PCVector& pc_list) {
    return std::count_if(
      pc_list.begin(), pc_list.end(),
      [](const PieceType& pc) { return is_pawn_like<Rev>(pc); }
    );
  }
...
int Search::run_single_(const PCVector& pc_list) {
  ...
  return search_(pc_list_sorted, pawn, pawn_v,
    allOneBB(), allZeroBB(), 0, 0,
    pieces_log, ans_sfens_, false);
}
...
bool judge_placeable(Bitboard no_effect_bb,
  ...
  return (
    next_count >= pawn
    && no_effect_bb.popCount() - 2 * pawn >= stone);
}
...
bool judge_placeable_dual(Bitboard no_effect_bb,
  ...
  do {
    ...
    no_effect_bb &= ~(next_placement | next_placement_v |
      next_placement.up<false>() | next_placement_v.up<true>());
    pawn -= next_count;
    ...
    next_placement = (
      ~pieces.down<false>() &
      ~no_effect_bb.up<false>() & no_effect_bb & no_effect_bb.down<false>().down<false>()
    );
  ...
  next_placement = (
    ~pieces.down<false>() &
    ~no_effect_bb.up<false>() & no_effect_bb);
  next_count = next_placement.popCount();
  while (next_count > 0 && next_count < pawn) {
...
template bool judge_placeable<false>(Bitboard no_effect_bb,
    int pawn, int stone,
    Bitboard pawn_allowed);
template bool judge_placeable<true>(Bitboard no_effect_bb,
    int pawn, int stone,
    Bitboard pawn_allowed);
